module ArgoPump::hook_factory {
    ...
    public(package) fun create_pool(
        ...
        if (hook_type == HOOK_VAULT) {
            ...
            let pool_addr = address_of(pool_signer);
            ArgoPump::vault::create_pool(pool_signer, assets, fee, stream, creator);
            let pool_meta = new_pool_meta(pool_addr, hook_type, assets);
            move_to(pool_signer, pool_meta);
            return cref
        };
        ...
        if (hook_type == HOOK_ADVANCED) {
            ...
            let cref = create_named_object(vault, seed);            let pool_signer = &generate_signer(&cref);
            let pool_addr = address_of(pool_signer);
            ArgoPump::advanced::create_pool(pool_signer, assets, fee, creator);
            let pool_meta = new_pool_meta(pool_addr, hook_type, assets);
            move_to(pool_signer, pool_meta);
            return cref
        };
    ...
    public(package) fun add_liquidity(
        ...
        if (pool_meta.hook_type == HOOK_BASIC) {
            let (amounts, mint_position) =
                ArgoPump::basic::add_liquidity(pool_signer, position_idx, stream, creator);
        ...
        if (pool_meta.hook_type == HOOK_VAULT) {
            let (amounts, mint_position) =
                ArgoPump::vault::deposit(pool_signer, position_idx, stream, creator);
            let assets = pool_meta.assets;
            let txs =
                range(0, amounts.length()).zip_map(
        ...
        if (pool_meta.hook_type == HOOK_ADVANCED) {
            let (amounts, mint_position) =
                ArgoPump::advanced::add_liquidity(
    ...
    public(package) fun remove_liquidity(
        ...
        if (pool_meta.hook_type == HOOK_BASIC) {
            let (amounts, burn_position) =
                ArgoPump::basic::remove_liquidity(pool_signer, position_idx, stream, creator);
            let assets = pool_meta.assets;
            let txs =
                range(0, amounts.length()).zip_map(
        ...
        if (pool_meta.hook_type == HOOK_VAULT) {
            let (amounts, burn_position) =
                ArgoPump::vault::withdraw(pool_signer, position_idx, stream, creator);
            let assets = pool_meta.assets;
            let txs =
                range(0, amounts.length()).zip_map(
        ...
        if (pool_meta.hook_type == HOOK_ADVANCED) {
            let (amounts, burn_position, rewards) =
                ArgoPump::advanced::remove_liquidity(
                    pool_signer, position_idx, stream, creator
                );
            let assets = pool_meta.assets;
            let txs =
    ...
    public(package) fun swap(
        ...
        if (pool_meta.hook_type == HOOK_BASIC) {
            let (a2b, amount_in, amount_out) =
                ArgoPump::basic::swap(pool_signer, stream, creator);
            let assets = pool_meta.assets;
            return if (a2b) {
        ...
        if (pool_meta.hook_type == HOOK_VAULT) {
            ArgoPump::vault::swap(pool_signer, stream, creator);
            return vector[]
        };

        if (pool_meta.hook_type == HOOK_ADVANCED) {
            let (i, j, dx, dy) = ArgoPump::advanced::swap(pool_signer, stream, creator);
            let assets = pool_meta.assets;
    ...
    public(package) fun collect_fee(
        ...
        let pool_meta = &mut PoolMeta[address_of(pool_signer)];
        if (pool_meta.hook_type == HOOK_VAULT) {
            let (amounts) = ArgoPump::vault::collect_fee(pool_signer, creator);
        ...
        if (pool_meta.hook_type == HOOK_ADVANCED) {
            let (amounts) =
                ArgoPump::advanced::collect_fee(pool_signer, position_idx, creator);
            let txs =
                pool_meta.assets.zip_map(
                    amounts, |asset, amount| tx(asset, amount as u64, false)
                );
    ...
    public(package) fun run_pool_op(
        ...
        if (pool_meta.hook_type == HOOK_ADVANCED) {
            ArgoPump::advanced::run_pool_op(pool_signer, stream);
            return;
        };
